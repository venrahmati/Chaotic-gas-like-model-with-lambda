...
def saving(N,z,z1,lan):
    N2 = N*N
    m = np.ones(N)
    for i in range(N2):
        dm =  (z1[i]+z1[i+1])*0.5*(1-lan)*m[z[i,1]] - (1-(z1[i]+z1[i+1])*0.5)*(1-lan)*m[z[i,0]]
        m[z[i,1]] = m[z[i,1]] - dm 
        m[z[i,0]] = m[z[i,0]] + dm 
    return m
...
plt.title('Money Distribution with chaotic agent selection & trade & equal saving parameter')
plt.legend()
